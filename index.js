...
const saveUsers = async (users) => {
  try {
    ...
    console.log('Users saved successfully to MongoDB');
  } catch (err) {
...
const saveContests = async (contests) => {
  try {
    ...
    console.log('Contests saved successfully to MongoDB');
  } catch (err) {
    console.error('コンテストの保存エラー:', err);
  } finally {
    await client.close();
  }
};
                ...
                ${
                  contests
                    .map(
                      (contest, index) => `
                    <li>
                        ${contest.title} (開始: ${new Date(
                        contest.startTime,
                      ).toLocaleString()}, 終了: ${new Date(
                        contest.endTime,
                      ).toLocaleString()})
                  ...
                  activeContestsWithIndex.length > 0
                    ? activeContestsWithIndex
                        ...
                        <li>
                            ...
                            <p>開始: ${start.toLocaleString()}</p>
                            <p>終了: ${end.toLocaleString()}</p>
                            <p>状態: ${status}</p>
                            <button onclick="window.location.href='/contest/${originalIndex}'" ${
                              status === '準備中' ? 'disabled' : ''
                            }>${status === '開催中' ? '参加' : '準備中'}</button>
                            ${
                        ...
                        ${
                          firstFA[id]
                            ? `FA: ${
                                ...
                                Math.floor((firstFA[id].time - startTime) / 1000),
                              )}`
                            : 'CA者なし'
                        }
                        </th>
                    ...
                    .map((rank, index) => {
                      ...
                      return `
                        <tr>
                            ...
                            <td>${rank.score}</td>
                            <td>${formatTime(
                              Math.floor(rank.lastCATime),
                            )}<br>+${rank.totalWABeforeCA}</td>
                            ${problemIds
                              .map((problemId) => {
                                const problem =
                                ...
                                if (problem.status === 'CA') {
                                  return `<td style="background-color: #90ee90;">${formatTime(
                                    Math.floor(problem.time),
                                  )}<br>+${problem.waCount}</td>`;
  ...
  let content = `
        <section class="hero">
            ...
            <div class="problem-display">
                ...
                ${
                  problem.image
                    ? `<p>画像: <img src="/uploads/${problem.image}" alt="Problem Image" style="max-width: 300px;"></p>`
                    : ''
                }
            </div>
    `;
...
app.post(
  '/admin/contest-details/:contestId/add-writer-tester',
  async (req, res) => {
    const user = getUserFromCookie(req);
    if (!user) return res.redirect('/login');
    const contests = await loadContests();
...
app.get('/admin/edit-contest/:contestId', async (req, res) => {
  ...
  const content = `
        <section class="form-container">
            ...
            <form method="POST" action="/admin/edit-contest/${contestId}" enctype="multipart/form-data">
                <label>コンテスト名:</label><br>
                  ...
                  .map((problemId) => {
                    ...
                    return `
                        <div>
                            ...
                            <label>画像:</label><br>
                            <input type="file" name="image_${problemId}" accept="image/*"><br>
                            ${
                              problem.image
                                ? `<img src="/uploads/${problem.image}" alt="Problem Image" style="max-width: 300px;">`
                                : ''
                            }
                        </div>
                    `;
...
app.post('/admin/edit-contest/:contestId', upload.any(), async (req, res) => {
  const user = getUserFromCookie(req);
  if (!user) return res.redirect('/login');
  const contests = await loadContests();
  const contestId = parseInt(req.params.contestId);
...
app.get('/admin/problem/:contestId/:problemId', async (req, res) => {
  ...
  const content = `
        <section class="problem-section">
            ...
            <form method="POST" action="/admin/problem/${contestId}/${problemId}" class="problem-form" enctype="multipart/form-data">
...
app.post(
  '/admin/problem/:contestId/:problemId',
  upload.single('image'),
  async (req, res) => {
    const user = getUserFromCookie(req);
    if (!user) return res.redirect('/login');
    ...
    let image = problemIndex !== -1 ? contest.problems[problemIndex].image || '' : '';
  ...
  const content = `
                      ...
                      .map(
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" ${
                                  u.isAdmin ? 'checked' : ''
                                } data-index="${index}" onchange="toggleAdmin(this)">
                            </td>
                            <td>
                                <ul>
                                    ${contests
                                      .map((contest, contestIndex) => {
                                        const isManager =
                                          contest.managers &&
                                          contest.managers.includes(u.username);
                                        return `
                                                <li>
                                                    ${contest.title}
                                                    <form action="/admin/toggle-contest-manager/${index}/${contestIndex}" method="POST" style="display:inline;">
                            ...
                            <td>
                                <form action="/admin/edit-user/${index}" method="GET" style="display:inline;">
                                    <button type="submit">編集</button>
                                </form>
                                <form action="/admin/delete-user/${index}" method="POST" style="display:inline;">
                                    <button type="submit">削除</button>
                                </form>
                            </td>
                        </tr>
        ...
        <script>
            function toggleAdmin(checkbox) {
                ...
                fetch(\`/admin/toggle-admin/\${index}\`, {
                    method: 'POST',
...
app.post('/admin/add-user', async (req, res) => {
  ...
  if (users.find((u) => u.username === username)) {
    return res.send('ユーザー名が既に存在します <a href="/admin/users">戻る</a>');
  }
  users.push({ username, password, isAdmin: isAdmin === 'true' });
  await saveUsers(users);
  res.redirect('/admin/users');
});

app.get('/admin/edit-user/:index', async (req, res) => {
  const user = getUserFromCookie(req);
  if (!user || !user.isAdmin) return res.redirect('/login');
  const users = await loadUsers();
  const index = parseInt(req.params.index);
  if (isNaN(index) || index < 0 || index >= users.length) {
    return res.status(404).send('無効なユーザーIDです');
  }
  const u = users[index];
  const nav = generateNav(user);
  const content = `
        <section class="form-container">
            <h2>ユーザー編集</h2>
            <form method="POST" action="/admin/edit-user/${index}">
                <label>ユーザー名:</label><br>
                <input type="text" name="username" value="${u.username}" required><br>
...
app.post('/admin/edit-user/:index', async (req, res) => {
  const user = getUserFromCookie(req);
  if (!user || !user.isAdmin) return res.redirect('/login');
  const users = await loadUsers();
  const index = parseInt(req.params.index);
  ...
  const existingUser = users.find(
    (u) => u.username === username && users.indexOf(u) !== index,
  );
  if (existingUser) {
    return res.send('ユーザー名が既に存在します <a href="/admin/users">戻る</a>');
  }
  ...
  users[index].isAdmin = isAdmin === 'true';
  await saveUsers(users);
  res.redirect('/admin/users');
});

app.post('/admin/delete-user/:index', async (req, res) => {
  const user = getUserFromCookie(req);
  if (!user || !user.isAdmin) return res.redirect('/login');
  const users = await loadUsers();
  const index = parseInt(req.params.index);
  ...
  if (users[index].username === user.username) {
    return res.send('自分自身は削除できません <a href="/admin/users">戻る</a>');
  }
...
app.post('/admin/toggle-admin/:index', async (req, res) => {
  const user = getUserFromCookie(req);
  if (!user || !user.isAdmin) return res.redirect('/login');
  const users = await loadUsers();
  const index = parseInt(req.params.index);
  if (isNaN(index) || index < 0 || index >= users.length) {
    return res.status(404).json({ error: '無効なユーザーIDです' });
  }
...
app.post('/admin/toggle-contest-manager/:userIndex/:contestIndex', async (req, res) => {
  const user = getUserFromCookie(req);
  if (!user || !user.isAdmin) return res.redirect('/login');
  const users = await loadUsers();
  const contests = await loadContests();
  const userIndex = parseInt(req.params.userIndex);
  const contestIndex = parseInt(req.params.contestIndex);
  ...
  if (!contest.managers) contest.managers = [];
  const isManager = contest.managers.includes(targetUser.username);
  if (isManager) {
    contest.managers = contest.managers.filter(
      (username) => username !== targetUser.username,
    );
  } else {
    contest.managers.push(targetUser.username);
  }
  await saveContests(contests);
  res.redirect('/admin/users');
});
