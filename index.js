...
// 環境変数が未定義の場合、警告を表示（サーバーは停止しない）
if (!MONGO_URI) {
    console.error('Warning: MONGO_URI is not defined. MongoDB connection will fail.');
    console.error('Ensure MONGO_URI is set in .env file or Vercel environment variables.');
}
...
console.log("MONGO_URI for MongoClient:", uri); // デバッグ用

// MONGO_URI が存在する場合のみクライアントを初期化
let client;
try {
    if (uri) {
        client = new MongoClient(uri);
    } else {
        console.warn("MONGO_URI is not set. MongoDB client will not be initialized.");
    }
} catch (err) {
    console.error("Error initializing MongoClient:", err);
}

async function connectToMongo() {
    if (!client) {
        throw new Error("MongoDB client is not initialized. Check MONGO_URI.");
    }
    if (!db) {
...
app.get('/', async (req, res) => {
    try {
        const user = await getUserFromCookie(req); // MongoDB依存
        const nav = generateNav(user);
        const content = `
            <section class="hero">
                <h2>TOHO Math Contestにようこそ！</h2>
        ...
        if (err.message.includes("MongoDB client is not initialized")) {
            res.status(503).send("サーバーは動作していますが、MongoDBに接続できません。管理者に連絡してください。");
        } else {
...
app.listen(PORT, async () => {
    ...
    if (client) {
        try {
            ...
            console.error('サーバー起動後のMongoDB接続エラー:', err);
            console.warn("Server is running, but MongoDB connection failed. Routes requiring MongoDB will not work.");
        }
    } else {
        console.warn("MongoDB client is not initialized. Routes requiring MongoDB will not work.");
    }
});